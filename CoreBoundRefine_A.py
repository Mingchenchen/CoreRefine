...
for x,y in lis:
	count=count+1
	if start <= count <= end:				#Do not include the morif sequence in the refine code
		continue
		...
		if y<=25 and (x=='-' or x=='T' or x=='S'):	#Core Loop
			...
			print(count,'A PIKAA AVILFYWGNQSTPDEKR')
			...
			boundery.append(count)
			print(count,'A PIKAA AVILWQEKFM')
			...
			boundery.append(count)
			print(count,'A PIKAA AVILFYWQTM')
